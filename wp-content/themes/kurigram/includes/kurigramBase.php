<?php
if(!class_exists("kurigramBase")) {
	class kurigramBase {
    	public $key = "2C5DB2C9AD9B1663";
    	private $product_id = "5";
    	private $product_base = "kurigram";
    	private $server_host = "https://www.witrsv.itkurigram.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_kurigram_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=kurigram_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
